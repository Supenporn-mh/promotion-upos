<template>
  <div class="bg-white rounded-[24px] shadow-xl shadow-gray-200/40 p-6 md:p-10 max-w-4xl mx-auto mb-10 border border-gray-100 font-sans">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mb-8">
      ...
      <!-- Section 1: ชื่อและรหัสโปรโมชั่น -->
      ...
      <div class="col-span-1 md:col-span-2">
        <label class="mb-1.5 text-[11px] font-semibold text-gray-400 uppercase tracking-widest ml-1 block">รูปภาพโปรโมชั่น</label>
        <div 
      ...
      <!-- Section 2: สิทธิ & ประเภทโปรโมชั่น -->
      ...
      <!-- Section 3: สินค้าเงื่อนไข (สิ่งที่ต้องซื้อเพื่อรับสิทธิ์) -->
      <div class="col-span-1 md:col-span-2 p-6 rounded-[20px] bg-white border border-gray-100 shadow-sm shadow-blue-500/5 flex flex-col gap-4">
        <h3 class="font-semibold text-gray-800 text-[13px] tracking-wide flex items-center gap-2 uppercase mb-1">
          <ph-shopping-cart :size="18" weight="bold" class="text-primary-main"/> เงื่อนไขการซื้อ (ซื้ออะไรเพื่อรับสิทธิ์)
        </h3>
        ...
        <div class="flex flex-col md:flex-row gap-6">
          <div class="w-full md:w-1/3">
            <BaseSelect 
              ...
              :options="[{label:'ซื้อครบตามยอด (บาท)', value:'TOTAL_SALES'}, {label:'ระบุรายสินค้า (SKU)', value:'ITEM'}, {label:'ยกหมวดหมู่สินค้า', value:'CATEGORY'}]"
              required
            />
          </div>
          
          <!-- Dropdown Selector - Condition Targets -->
          <div class="w-full md:w-2/3 flex flex-col relative" v-if="form.targetType !== 'TOTAL_SALES'">
            <label class="mb-1.5 text-[11px] font-semibold text-gray-500 uppercase tracking-widest ml-1 text-right">
              ค้นหาและระบุ{{ form.targetType === 'CATEGORY' ? 'หมวดหมู่' : 'รายการสินค้า' }}
            </label>
            
            <div 
              class="min-h-[48px] w-full bg-gray-50 border transition-all cursor-pointer rounded-xl p-2 flex flex-wrap gap-2 shadow-inner"
              :class="showDropdowns.target ? 'border-primary-main ring-2 ring-primary-main/10 bg-white' : 'border-transparent hover:border-gray-200'"
              @click.stop="toggleDropdown('target')"
            >
               <div v-if="selectedProducts.length === 0" class="flex items-center text-gray-400 text-sm px-2">
                 + เลือก{{ form.targetType === 'CATEGORY' ? 'หมวดหมู่' : 'สินค้า' }}...
               </div>
               <div v-for="item in selectedProducts" :key="item.id" class="bg-primary-main/10 text-primary-main text-[11px] px-2.5 py-1.5 rounded-lg flex items-center gap-1.5 font-semibold border border-primary-main/20">
                 {{ item.name }} 
                 <ph-x-circle weight="fill" class="cursor-pointer opacity-70 hover:opacity-100 transition-opacity" @click.stop="toggleTargetItem(item)" />
               </div>
          ...
          <div class="w-full md:w-2/3 flex flex-col" v-if="form.targetType === 'TOTAL_SALES'">
             <BaseInput id="promo_min_sales" label="ยอดซื้อสุทธิขั้นต่ำ (บาท)" type="number" v-model="form.minAmount" />
          </div>
        </div>
      </div>
      
      <!-- Validation text for quantities -->
      <div class="col-span-1 md:col-span-2 p-6 rounded-[20px] bg-white border border-gray-100 shadow-sm shadow-blue-500/5" v-if="form.targetType !== 'TOTAL_SALES' && selectedProducts.length > 0">
        <label class="mb-3 block text-[11px] font-semibold text-gray-500 uppercase tracking-widest ml-1">ระบุจำนวนชิ้นขั้นต่ำที่ต้องซื้อ</label>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div v-for="item in selectedProducts" :key="item.id">
            <div class="flex justify-between items-center bg-gray-50 rounded-xl p-2 px-3 border border-transparent focus-within:border-primary-main transition-all" :class="item.qty <= 0 ? 'border-red-300 bg-red-50/30' : ''">
              <span class="text-gray-700 text-[12px] font-semibold truncate pr-3">{{ item.name }}</span>
              <div class="flex items-center gap-2">
                <input type="number" class="w-16 border-0 bg-white shadow-inner rounded-lg text-center py-1.5 text-sm font-semibold outline-none ring-1 ring-gray-100 focus:ring-primary-main" v-model.number="item.qty">
                <span class="text-gray-400 text-[10px] font-semibold">ชิ้น</span>
              </div>
            </div>
      ...
      <div class="col-span-1 md:col-span-2 p-6 rounded-[20px] bg-white border border-gray-100 shadow-sm shadow-blue-500/5 mt-2" v-if="form.type !== 'DISCOUNT'">
        ...
        <div class="flex flex-col gap-6">
           ...
           <div class="flex gap-4 border-b border-gray-100 pb-4">
              <button 
                v-for="bt in [{l:'รายสินค้า',v:'ITEM'},{l:'หมวดหมู่',v:'CATEGORY'},{l:'กำหนดเอง',v:'CUSTOM'}]" :key="bt.v"
           ...
           <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
              ...
              <div class="col-span-1 md:col-span-7 flex flex-col relative" v-if="form.rewardType !== 'CUSTOM'">
              ...
              <div class="col-span-1 md:col-span-7" v-else>
                 <BaseInput id="rew_custom" label="ระบุชื่อสินค้า/ของแถม" v-model="form.rewardCustomName" />
              </div>
      ...
      <div class="col-span-1 md:col-span-2 p-6 rounded-[20px] bg-white border border-gray-100 shadow-sm shadow-blue-500/5 mt-2 flex flex-col gap-4">
         <h3 class="font-semibold text-gray-800 text-[13px] tracking-wide flex items-center gap-2 uppercase mb-1">
           <ph-calendar-blank :size="18" weight="bold" class="text-primary-main" /> ระยะเวลาโปรโมชั่น
         </h3>
            ...
            <div v-for="q in [{k:'quota',l:'โควต้ารวม'},{k:'perOrder',l:'ต่อออเดอร์'},{k:'perCustomer',l:'ต่อคน'}]" :key="q.k" class="bg-white p-3 rounded-2xl border border-gray-100 shadow-sm">
               <div class="flex justify-between items-center mb-2">
                  ...
                  <label class="flex items-center gap-1 cursor-pointer">
                    <input type="checkbox" v-model="unlimited[q.k]" class="hidden peer">
...
const form = reactive({
  name: '', code: '', eligibility: 'ALL', type: 'DISCOUNT', targetType: 'TOTAL_SALES', minAmount: 500,
  discountType: 'PERCENT', discountValue: 0, maxDiscount: 0,
  rewardType: 'ITEM', rewardSelectionRule: 'ALL', rewardItemId: '', rewardCustomName: '', rewardQty: 1, redemptionPrice: 0,
...
const unlimited = reactive({ quota: false, perOrder: true, perCustomer: true })
